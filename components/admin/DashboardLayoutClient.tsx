...
export function DashboardLayoutClient({ children, userEmail }: DashboardLayoutClientProps) {
  ...
  // Handle outside clicks to close user menu & floating action button
  useEffect(() => {
    ...
    <div className="flex h-screen flex-col overflow-hidden bg-[#FAFAFA] font-sans antialiased text-[#555555]">
      ...
      <header className="sticky top-0 z-40 flex h-14 w-full shrink-0 items-center justify-between border-b border-[#ECECEC] bg-white px-4 shadow-sm backdrop-blur-md">
        ...
        <button
          ...
          className="mx-4 flex max-w-sm flex-1 items-center justify-between gap-3 rounded-lg border border-[#ECECEC] bg-[#FAFAFA] px-3 py-1.5 text-left text-xs text-[#888888] shadow-inner transition hover:border-[#DCDCDC] hover:bg-neutral-50 focus:outline-none focus:ring-1 focus:ring-[#E31B23]/30"
        >
          <div className="flex items-center gap-2">
            ...
            <span>Search products, orders, counties...</span>
          </div>
          <kbd className="hidden rounded bg-white px-1.5 py-0.5 text-[10px] font-semibold text-[#888888] shadow-sm border border-[#ECECEC] sm:inline-block">Ctrl + K</kbd>
        </button>
      ...
      <div className="md:hidden fixed bottom-0 left-0 right-0 z-40 h-16 bg-white border-t border-[#ECECEC] flex items-center justify-around px-2 shadow-[0_-4px_10px_rgba(0,0,0,0.03)]">
        ...
        <Link
          ...
          className={cn(
            "flex flex-col items-center justify-center w-14 h-full text-[10px] font-bold gap-1 transition-all",
            isLinkActive("/dashboard/products") ? "text-[#E31B23]" : "text-[#888888] hover:text-[#111111]"
          )}
        >
          <svg className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
            <path strokeLinecap="round" strokeLinejoin="round" d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.247 18 16.5 18c-1.746 0-3.332.477-4.5 1.253" />
      ...
      {isSearchOpen && (
        <div className="fixed inset-0 z-50 flex items-start justify-center p-4 pt-16 sm:pt-28">
          ...
          <div className="relative w-full max-w-lg rounded-xl border border-[#ECECEC] bg-white shadow-2xl ring-1 ring-black/5 overflow-hidden animate-in zoom-in-95 duration-150 flex flex-col max-h-[480px]">
            {/* Input fields header */}
            ...
            <div className="flex-1 overflow-y-auto custom-scrollbar p-2">
              {searchQuery.trim().length < 2 ? (
                ...
                <div className="space-y-3">
                  ...
                  {(["order", "product", "category", "session"] as const).map((catType) => {
