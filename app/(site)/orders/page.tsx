...
import { lookupOrderAction, checkPdfStatusAction, type LookupOrderState } from "@/lib/actions/submit-exam-order";
...
export default function OrderLookupPage() {
  ...
  useEffect(() => {
    if (state.status === "success") {
      setPdfPath(state.pdfStoragePath);
      setPdfFailed(state.pdfGenerationFailed);
      if (!state.pdfStoragePath && !state.pdfGenerationFailed) {
        setIsPolling(true);
      } else {
        setIsPolling(false);
      }
  ...
  useEffect(() => {
    if (!isPolling || state.status !== "success") return;

    const interval = setInterval(async () => {
      try {
        const res = await checkPdfStatusAction(state.orderNumber, state.phone);
        if (res.pdfStoragePath) {
          setPdfPath(res.pdfStoragePath);
          setIsPolling(false);
        } else if (res.pdfGenerationFailed) {
          setPdfFailed(true);
          setIsPolling(false);
        }
      } catch (err) {
              ...
              <div className="overflow-hidden rounded-3xl border border-neutral-200 bg-white p-6 shadow-[var(--shadow-sm)] sm:p-8">
                ...
                <div className="mt-8 flex flex-col gap-3 sm:flex-row border-t border-neutral-100 pt-6">
                  {pdfPath ? (
                    ...
                    <div className="flex-1 flex items-center justify-center rounded-xl border border-red-200 bg-red-50 px-4 py-3 text-xs font-bold text-red-800 text-center">
                      We encountered an issue preparing your PDF. Please contact support.
                    </div>
                  ) : (
                    <div className="flex-1 flex items-center justify-center gap-2 rounded-xl bg-neutral-100 border border-neutral-200 px-6 text-sm font-semibold text-neutral-500 py-3 animate-pulse">
